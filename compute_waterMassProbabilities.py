...
def compute_prob_unweighted_bootstrapped(dfwm, save=False, savename='untitled.png', reps=10000,
    ...
    for i in range(2):
        ...
        MC_prob_DSW_cond_noCDW[MC_prob_DSW_cond_noCDW > 1] = 1 # random sampling can sometimes return count exceeding total count, which will give a probability exceeding value of 1.
        MC_prob_DSW_cond_noCDW[MC_prob_DSW_cond_noCDW < 0] = 0 # random sampling can sometimes return count exceeding total count, which will give a probability exceeding value of 1.        
        ...
        MC_prob_CDW_cond_noDSW[MC_prob_CDW_cond_noDSW > 1] = 1
        MC_prob_CDW_cond_noDSW[MC_prob_CDW_cond_noDSW < 0] = 0        
        ...
        MC_prob_DSW_cond_CDW[MC_prob_DSW_cond_CDW > 1] = 1
        MC_prob_DSW_cond_CDW[MC_prob_DSW_cond_CDW < 0] = 0        
        ...
        MC_prob_CDW_cond_DSW[MC_prob_CDW_cond_DSW > 1] = 1
        MC_prob_CDW_cond_DSW[MC_prob_CDW_cond_DSW < 0] = 0        
...
def compute_prob_unweighted_bootstrapped_v2(dfwm, save=False, savename='untitled.png', reps=10000,
                                        show=True, width=90/25.4, height=90/25.4):
    matplotlib.rcParams.update({'font.size': 8})        # setting fontsize for plot elements        
    wm1='DSW'
    wm2='CDW'
    ...
    for i in range(2):
        ...
        prob_DSW_cap_NoCDW = len(seldDSW[seldCDW < 20].nonzero()[0]) / totalLength # P(DSW intersection noCDW)
        prob_noCDW = len(seldCDW[seldCDW < 20]) / totalLength
        prob_DSW_cond_noCDW[i] = prob_DSW_cap_NoCDW / prob_noCDW

        randidxr = np.random.choice(len(seldDSW[seldCDW < 20]), (len(seldDSW[seldCDW < 20]), reps), replace=True)
        MC_DSW_cap_NoCDW = seldDSW[seldCDW < 20][randidxr]
        MC_prob_DSW_cap_noCDW = np.count_nonzero(MC_DSW_cap_NoCDW, axis=0) / totalLength

        randidxr = np.random.choice(len(seldCDW), (len(seldCDW), reps), replace=True)
        MC_CDW = seldCDW[randidxr]
        MC_prob_noCDW = ma.count(ma.masked_array(MC_CDW, mask=(MC_CDW > 20) ), axis=0) / totalLength
        MC_prob_DSW_cond_noCDW = MC_prob_DSW_cap_noCDW / MC_prob_noCDW
        MC_prob_DSW_cond_noCDW[MC_prob_DSW_cond_noCDW > 1] = 1 # random sampling can sometimes return count exceeding total count, which will give a probability exceeding value of 1.
        MC_prob_DSW_cond_noCDW[MC_prob_DSW_cond_noCDW < 0] = 0 # random sampling can sometimes return count exceeding total count, which will give a probability exceeding value of 1.        
        prob_DSW_cond_noCDW_yerr[i] = abs(prob_DSW_cond_noCDW[i] - np.percentile(MC_prob_DSW_cond_noCDW, [2.5, 97.5]))
        ...
        prob_CDW_cap_NoDSW = len(seldCDW[seldDSW < 20].nonzero()[0]) / float(totalLength)
        prob_noDSW = len(seldDSW[seldDSW < 20]) / float(totalLength)
        prob_CDW_cond_noDSW[i] = prob_CDW_cap_NoDSW / prob_noDSW

        randidxr = np.random.choice(len(seldCDW[seldDSW < 20]), (len(seldCDW[seldDSW < 20]), reps), replace=True)
        MC_CDW_cap_NoDSW = seldCDW[seldDSW < 20][randidxr]
        MC_prob_CDW_cap_noDSW = np.count_nonzero(MC_CDW_cap_NoDSW, axis=0) / totalLength

        randidxr = np.random.choice(len(seldDSW), (len(seldDSW), reps), replace=True)
        MC_DSW = seldDSW[randidxr]
        MC_prob_noDSW = ma.count(ma.masked_array(MC_DSW, mask=(MC_DSW > 20) ), axis=0) / totalLength
        MC_prob_CDW_cond_noDSW = MC_prob_CDW_cap_noDSW / MC_prob_noDSW
        MC_prob_CDW_cond_noDSW[MC_prob_CDW_cond_noDSW > 1] = 1
        MC_prob_CDW_cond_noDSW[MC_prob_CDW_cond_noDSW < 0] = 0        
        prob_CDW_cond_noDSW_yerr[i] = abs(prob_CDW_cond_noDSW[i] - np.percentile(MC_prob_CDW_cond_noDSW, [2.5, 97.5]))
        ...
        prob_DSW_cap_CDW = len(seldDSW[seldCDW > 20].nonzero()[0]) / totalLength
        prob_CDW = len(seldCDW.nonzero()[0]) / totalLength
        prob_DSW_cond_CDW[i] = prob_DSW_cap_CDW / prob_CDW

        randidxr = np.random.choice(len(seldDSW[seldCDW > 20]), (len(seldDSW[seldCDW > 20]), reps), replace=True)
        MC_DSW_cap_CDW = seldDSW[seldCDW > 20][randidxr]
        MC_prob_DSW_cap_CDW = np.count_nonzero(MC_DSW_cap_CDW, axis=0) / totalLength

        MC_prob_CDW = ma.count(ma.masked_array(MC_CDW, mask=(MC_CDW < 20) ), axis=0) / totalLength
        MC_prob_DSW_cond_CDW = MC_prob_DSW_cap_CDW / MC_prob_CDW
        MC_prob_DSW_cond_CDW[MC_prob_DSW_cond_CDW > 1] = 1
        MC_prob_DSW_cond_CDW[MC_prob_DSW_cond_CDW < 0] = 0        
        prob_DSW_cond_CDW_yerr[i] = abs(prob_DSW_cond_CDW[i] - np.percentile(MC_prob_DSW_cond_CDW, [2.5, 97.5]))
        ...
        prob_CDW_cap_DSW = len(seldCDW[seldDSW > 20].nonzero()[0]) / totalLength
        prob_DSW = len(seldDSW[seldDSW > 20]) / totalLength
        prob_CDW_cond_DSW[i] = prob_CDW_cap_DSW / prob_DSW

        randidxr = np.random.choice(len(seldCDW[seldDSW > 20]), (len(seldCDW[seldDSW > 20]), reps), replace=True)
        MC_CDW_cap_DSW = seldCDW[seldDSW > 20][randidxr]
        MC_prob_CDW_cap_DSW = np.count_nonzero(MC_CDW_cap_DSW, axis=0) / totalLength

        MC_prob_DSW = ma.count(ma.masked_array(MC_DSW, mask=(MC_DSW < 20) ), axis=0) / totalLength
        MC_prob_CDW_cond_DSW = MC_prob_CDW_cap_DSW / MC_prob_DSW
        MC_prob_CDW_cond_DSW[MC_prob_CDW_cond_DSW > 1] = 1
        MC_prob_CDW_cond_DSW[MC_prob_CDW_cond_DSW < 0] = 0        
        prob_CDW_cond_DSW_yerr[i] = abs(prob_CDW_cond_DSW[i] - np.percentile(MC_prob_CDW_cond_DSW, [2.5, 97.5]))
...
def compute_prob_weighted_bootstrapped(dfwm, save=False, savename='untitled.png', reps=10000,
                                        show=True, width=90/25.4, height=90/25.4):
    matplotlib.rcParams.update({'font.size': 8})        # setting fontsize for plot elements        
    wm1='DSW'
    wm2='CDW'
    ...
    for i in range(2):
        selMask = ( (dfwm.month.isin(season[i]) ) & (dfwm["DSW"].notnull()) )
        totalVolume = np.nansum(dfwm.loc[selMask, "zlowest"])
        totalLength = float(len(dfwm.loc[selMask]))
        seldDSW = dfwm[selMask].DSW.values
        seldCDW = dfwm[selMask].CDW.values
        
        prob_DSW_cap_NoCDW = np.nansum(seldDSW[(seldCDW < 20)]) / totalVolume # P(DSW intersection noCDW)
        prob_CDW = np.nansum(seldCDW) / totalVolume
        prob_noCDW = 1 - prob_CDW
        prob_DSW_cond_noCDW[i] = prob_DSW_cap_NoCDW / prob_noCDW

        randidxr = np.random.choice(len(seldDSW[seldCDW < 20]), (len(seldDSW[seldCDW < 20]), reps), replace=True)
        MC_DSW_cap_NoCDW = seldDSW[seldCDW < 20][randidxr]
        MC_prob_DSW_cap_noCDW = np.nansum(MC_DSW_cap_NoCDW, axis=0) / totalVolume

        randidxr = np.random.choice(len(seldCDW), (len(seldCDW), reps), replace=True)
        MC_CDW = seldCDW[randidxr]
        MC_prob_CDW = np.nansum(MC_CDW, axis=0)  / totalVolume        
        MC_prob_noCDW = 1 - MC_prob_CDW
        MC_prob_DSW_cond_noCDW = MC_prob_DSW_cap_noCDW / MC_prob_noCDW
        MC_prob_DSW_cond_noCDW[MC_prob_DSW_cond_noCDW > 1] = 1 # random sampling can sometimes return count exceeding total count, which will give a probability exceeding value of 1.
        MC_prob_DSW_cond_noCDW[MC_prob_DSW_cond_noCDW < 0] = 0 # random sampling can sometimes return count exceeding total count, which will give a probability exceeding value of 1.        
        prob_DSW_cond_noCDW_yerr[i] = abs(prob_DSW_cond_noCDW[i] - np.percentile(MC_prob_DSW_cond_noCDW, [2.5, 97.5]))
        ...
        prob_CDW_cap_NoDSW = np.nansum(seldCDW[seldDSW < 20]) / totalVolume
        prob_DSW = np.nansum(seldDSW[seldDSW > 20]) / totalVolume
        prob_noDSW = 1 - prob_DSW
        prob_CDW_cond_noDSW[i] = prob_CDW_cap_NoDSW / prob_noDSW

        randidxr = np.random.choice(len(seldCDW[seldDSW < 20]), (len(seldCDW[seldDSW < 20]), reps), replace=True)
        MC_CDW_cap_NoDSW = seldCDW[seldDSW < 20][randidxr]
        MC_prob_CDW_cap_noDSW = np.nansum(MC_CDW_cap_NoDSW, axis=0) / totalVolume

        randidxr = np.random.choice(len(seldDSW), (len(seldDSW), reps), replace=True)
        MC_DSW = seldDSW[randidxr]
        MC_prob_DSW = np.nansum(MC_DSW, axis=0) / totalVolume        
        MC_prob_noDSW = 1 - MC_prob_DSW
        MC_prob_CDW_cond_noDSW = MC_prob_CDW_cap_noDSW / MC_prob_noDSW
        MC_prob_CDW_cond_noDSW[MC_prob_CDW_cond_noDSW > 1] = 1
        MC_prob_CDW_cond_noDSW[MC_prob_CDW_cond_noDSW < 0] = 0        
        prob_CDW_cond_noDSW_yerr[i] = abs(prob_CDW_cond_noDSW[i] - np.percentile(MC_prob_CDW_cond_noDSW, [2.5, 97.5]))
        ...
        prob_DSW_cap_CDW = np.nansum(seldDSW[seldCDW > 20]) / totalVolume
        
        prob_DSW_cond_CDW[i] = prob_DSW_cap_CDW / prob_CDW

        randidxr = np.random.choice(len(seldDSW[seldCDW > 20]), (len(seldDSW[seldCDW > 20]), reps), replace=True)
        MC_DSW_cap_CDW = seldDSW[seldCDW > 0][randidxr]
        MC_prob_DSW_cap_CDW = np.nansum(MC_DSW_cap_CDW, axis=0) / totalVolume
        ...
        prob_CDW_cap_DSW = np.nansum(seldCDW[seldDSW > 20]) / totalVolume
        
        prob_CDW_cond_DSW[i] = prob_CDW_cap_DSW / prob_DSW

        randidxr = np.random.choice(len(seldCDW[seldDSW > 20]), (len(seldCDW[seldDSW > 20]), reps), replace=True)
        MC_CDW_cap_DSW = seldCDW[seldDSW > 20][randidxr]
        MC_prob_CDW_cap_DSW = np.nansum(MC_CDW_cap_DSW, axis=0) / totalVolume
