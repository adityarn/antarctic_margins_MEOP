...
lonbins = np.arange(0,361,2)

ax = plt.subplot(gs[0,0])
dfsel = df.SHELF_BREAK_PROFILE & (df.ECHODEPTH > -1500) #& sel_months(df, [12,1,2,3,4,5])
OHC_SB = OHC.compute_OHC(df[dfsel], ax, h_b=-500, ymax=7e9, lon_bins= lonbins)
ax.set_ylabel("Ocean Heat Content Anomaly (Jm$^{-2}$)")
ax.set_ylim(0, 7e9)

dfsel = df.SHELF_BREAK_PROFILE & (df.ECHODEPTH < -1500) #& sel_months(df, [12,1,2,3,4,5])
ax = plt.subplot(gs[0,1])
OHC_2 = OHC.compute_OHC(df[dfsel], ax, h_b=-500, ymax=7e9, hide_yticks=True, lon_bins= lonbins)
ax.set_ylim(0, 7e9)
...
plt.savefig("./Images/slopeFront/OHC.pdf", dpi=600, bbox_inches='tight')
