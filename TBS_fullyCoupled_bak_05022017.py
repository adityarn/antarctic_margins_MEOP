...
class Keps_Solver(object):
    def __init__(self, N, LAT, DEPTH, PRESSGRAD_X, PRESSGRAD_Y, RHO, U10, V10, OUTDIR, OUTFILE, init_u=[0.1,0.1],init_v=[0.1,0.1], mode="quiet"):
        ...
        self.y_mom[0] = self.y[0] + self.dy1_bot*0.5
    ...
    def solver(self, MAXITER, DT, SAVE_INTERVAL, tol=1e-3):
        ...
        while(self.iter < maxiter ):
            ...
            if(self.mode == "verbose"):
                print("this is iteration number = "+str(self.iter)+", max(norm_k)= "+str(max(self.norm_k))+", norm_eps_max="+str(max(self.norm_eps[:-1]))+", norm_u_max="+str(max(self.norm_u))+", norm_v_max="+str(max(self.norm_v))+", zo_bt = "+str(self.z0)+", y+_bt = "+str(ymom_plus_bottom))
            if(save_int != 0):
