...
def plot_AcrossASF_woWinds(acASF, dfmg, wd=190/25.4, ht=230/25.4, savefig=False, savename="untitled.png", levels=[], xlat=False, show=True, MEOPDIR = "/media/sda7", p2pdist=True, fontsize=8,
                   region='Whole', bathy=None, plotBathy=True, llcrnrlon=-180, llcrnrlat=-90, urcrnrlon=+180, urcrnrlat=+90):
    matplotlib.rcParams.update({'font.size': fontsize})    
    plt.close(1)
    fig = plt.figure(1, figsize=(wd, ht))
    gs = gridspec.GridSpec(3, 2, height_ratios=[1, 0.25, 0.5], width_ratios=[1, 0.02], hspace=0., wspace=0.05)
    ...
    with fiona.open(shpfile) as records:
        geometries = [sgeom.shape(shp['geometry']) for shp in records]
    mapax.add_geometries(geometries, ccrs.PlateCarree(), edgecolor='k', facecolor='none')
    
    ISedgefname = MEOPDIR+"/Datasets/Shapefiles/AntIceShelfEdge/ne_10m_antarctic_ice_shelves_lines.shp"
    ISe_feature = ShapelyFeature(Reader(ISedgefname).geometries(), ccrs.PlateCarree(), facecolor='none', edgecolor="gray")
    mapax.add_feature(ISe_feature, zorder=3)
