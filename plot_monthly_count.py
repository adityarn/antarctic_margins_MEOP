...
def plot_counts_profs(df, WSO_source, WSO_prod, boxes=[], 
    ...
    for j in range(len(boxes)):
        ...
        for i in range(12):
            nprof_all[i] = len(df.loc[(df['JULD'].dt.month.isin([monthnos[i]]) & boxes[j]),'PROFILE_NUMBER'].unique())
            nprof_source[i] = len(df.loc[(df['JULD'].dt.month.isin([monthnos[i]]) &WSO_source & boxes[j]), 'PROFILE_NUMBER'].unique())
            nprof_prod[i] = len(df.loc[(df['JULD'].dt.month.isin([monthnos[i]]) &WSO_prod & boxes[j]), 'PROFILE_NUMBER'].unique())
